  ...
  return (
    <div className="min-h-screen flex flex-col animate-fade-in">
      <div className="flex items-center px-10 py-5.5">
        <span className="font-[family-name:var(--font-display)] text-[19px]">Sankofa</span>
        <button
          type="button"
        ...
        <div className="w-full max-w-[460px] bg-white rounded-3xl shadow-[0_12px_32px_rgba(46,43,37,0.16)] p-9">
          <div className="flex gap-1 bg-panel rounded-full p-1 mb-6.5">
            <button
              ...
              className={`flex-1 text-center py-2.5 rounded-full text-[13.5px] cursor-pointer ${
                authMode === "signup" ? "bg-ink text-paper font-semibold" : "text-ink"
              }`}
            >
              Sign up
            </button>
            <button
              ...
              onClick={() => setAuthMode("login")}
              className={`flex-1 text-center py-2.5 rounded-full text-[13.5px] cursor-pointer ${
                authMode === "login" ? "bg-ink text-paper font-semibold" : "text-ink"
              }`}
            >
              Log in
            </button>
          ...
          {authMode === "signup" && (
            <div className="mt-3.5">
              <label className="block text-xs mb-1.5 text-ink/70">Business name</label>
              <input
                value={companyName}
          ...
          <p className="text-[11.5px] opacity-55 mt-4.5 mb-0 leading-relaxed">
            By continuing, I confirm I&apos;m authorised to build a financial profile on this business&apos;s
            behalf.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 px-10 py-3.5 bg-ink text-paper">
        <span className="font-[family-name:var(--font-display)] text-sm">Sankofa</span>
        <span className="ml-auto text-[11.5px] opacity-60">SME Credit Readiness Assistant</span>
      </div>
    </div>
  );
}
