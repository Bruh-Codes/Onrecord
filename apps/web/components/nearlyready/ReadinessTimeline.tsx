	...
	return (
		<div className="flex gap-3 mb-1">
			<div className="flex flex-col items-center">
				<span
					className={`w-[9px] h-[9px] rounded-full shrink-0 ${done ? "bg-ink" : "bg-ink/25"}`}
				/>
				{!isLast && <span className="w-[1.4px] flex-1 bg-ink/25 min-h-8" />}
			</div>
			<div>
				<div
					className={`text-[13.5px] font-semibold ${done ? "" : "text-ink/50"}`}
				>
					{title}
				</div>
	...
	return (
		<div className="border border-border rounded-[20px] p-5.5">
			...
			<div className="text-[12.5px] opacity-70">
				Our team can help:{" "}
				<a href="mailto:help@onrecord.app">help@onrecord.app</a>
			</div>
		</div>
	);
}
