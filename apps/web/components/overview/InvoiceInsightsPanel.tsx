	...
	return (
		<div className="flex flex-wrap items-center gap-3 border-b border-border/60 py-3 text-sm">
			...
			<div className="text-right">
				...
				<div className="text-xs text-foreground/55">{item.payment_status ?? "Status unavailable"}</div>
			</div>
		</div>
	);
}
...
export function InvoiceInsightsPanel({ data }: { data: InvoiceInsights | undefined }) {
	if (!data || data.total_documents === 0) {
		return <div className="rounded-xl border border-border/70 p-5 text-sm text-foreground/60">No invoice documents have been extracted yet.</div>;
	}
		...
		<section className="mb-12">
			<div className="grid gap-5 mb-5 grid-cols-1 md:grid-cols-3">
				<div className="rounded-xl border border-border/70 p-4">
					<div className="text-[12px] uppercase text-foreground/50">Issued invoices</div>
					<div className="text-[24px] mt-2">{data.issued_count.toLocaleString()}</div>
				</div>
				<div className="rounded-xl border border-border/70 p-4">
					<div className="text-[12px] uppercase text-foreground/50">Received bills</div>
					<div className="text-[24px] mt-2">{data.received_count.toLocaleString()}</div>
				</div>
				<div className="rounded-xl border border-border/70 p-4">
					<div className="text-[12px] uppercase text-foreground/50">Currencies</div>
			...
			<div className="rounded-xl border border-border/70 px-4">
				<div className="py-3 text-xs tracking-wider uppercase text-foreground/45">Invoice evidence</div>
				{data.items.map((item) => <InvoiceRow key={item.document_id} item={item} />)}
			</div>
			<p className="mt-3 mb-0 text-xs text-foreground/55">Invoice totals represent amounts billed or received as documents. They are not added to cashflow until matched to a bank or MoMo transaction.</p>
		</section>
	);
}
