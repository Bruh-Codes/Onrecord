...
export function UploadedDocumentsList({
  ...
  if (loading) {
    return (
      <div className="py-4 space-y-2.5" aria-busy="true" aria-label="Loading your documents">
        {[0, 1, 2].map((i) => (
  ...
  return (
    <div>
      {documents.map((doc) => (
        <div key={doc.id} className="py-3 border-b border-border flex items-center gap-3.5">
          <div className="w-[34px] h-[34px] shrink-0 rounded-[10px] bg-panel flex items-center justify-center">
            <DocumentsIcon />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold">{doc.doc_type ?? "Uploaded document"}</div>
            <div className="text-xs opacity-60">{new Date(doc.created_at).toLocaleString()}</div>
          </div>
