...
export function TodayStats({
  ...
  const coverageColor = coveragePositive ? "var(--color-positive)" : "var(--color-negative)";

  const total = score?.total ?? 0;
  const band = score?.band ?? "not_ready";
  const bandLabel =
    band === "lender_ready" ? "Lender-ready" : band === "nearly_ready" ? "Nearly ready" : band === "developing" ? "Developing" : "Not ready";
  const scoreColor =
    total >= 65 ? "var(--color-positive)" : total >= 40 ? "var(--color-muted)" : "var(--color-negative)";

  const nextGap = openGaps.find((g) => g.kind === "missing_period") ?? openGaps[0];
  ...
  return (
    <div className="flex gap-12 flex-wrap">
      ...
      <div className="w-[220px] shrink-0">
        ...
        {nextGap ? (
          <>
            <div className="text-[16px] font-semibold mb-0.5 leading-snug">{nextGap.title}</div>
            <div className="text-xs opacity-55">{nextGap.detail ?? nextGap.code}</div>
          </>
        ) : (
          <>
