...
export function ReviewDetailPanel({
  ...
  return (
    <div className="bg-white rounded-[20px] p-5 shadow-[0_3px_10px_rgba(20,20,20,0.1)]">
      <div className="text-[11px] tracking-wider uppercase text-ink/55 mb-2.5">Selected item</div>
      ...
      <div className="text-[11px] text-ink/55 mb-1">Extracted field — {item.field}</div>
      <div className="font-[family-name:var(--font-display)] text-xl mb-3">{item.value}</div>
      <div className="flex justify-between text-[11.5px] mb-1">
        <span>Confidence</span>
        <span>{item.confidencePct}%</span>
      </div>
      <div className="h-1.5 rounded-full bg-[#dddddb] overflow-hidden mb-4.5">
        <div className="h-full rounded-full bg-ink" style={{ width: `${item.confidencePct}%` }} />
      </div>
