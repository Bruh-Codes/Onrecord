...
export function ReviewTable({
  ...
  return (
    <div className="bg-muted rounded-[20px] overflow-hidden">
      <table className="w-full border-collapse text-[13.5px]">
        <thead>
          <tr>
              ...
              <th
                key={h}
                className="text-left text-[10.5px] tracking-wider uppercase text-foreground/55 px-4 py-3 border-b border-foreground/16"
              >
                {h}
              </th>
            ))}
			...
			{items.map((item) => {
				...
				return (
					<tr key={item.id} onClick={() => onSelect(item.id)} className={`cursor-pointer ${selectedId === item.id ? "bg-accent" : ""}`}>
						...
						<td className="px-4 py-3 border-b border-foreground/8 font-semibold">{formatGhs(item.amount_pesewas)}</td>
						<td className="px-4 py-3 border-b border-foreground/8">
							{suggestion ? <Badge tone={suggestion.confidence >= 0.6 ? "positive" : "neutral"}>{suggestion.category_l1.replaceAll("_", " ")} · {Math.round(suggestion.confidence * 100)}%</Badge> : <span className="text-xs text-foreground/50">No confident suggestion</span>}
						</td>
					</tr>
				);
			})}
