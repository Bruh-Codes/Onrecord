...
export function getUnclassifiedValue(state: AppState): string {
	return UNCLASSIFIED_VALUE_BY_RESOLVED_COUNT[
		Math.min(resolvedCount(state), 3)
	];
}
	...
	return [
		{
			...
			detail: state.resolved.coverage
				? "12 of 12 months"
				: "9 of 12 months-3 Feb to 3 May missing",
			done: state.resolved.coverage,
			actionLabel: state.resolved.coverage
				? undefined
				: "Upload missing window",
		},
		{
			label: "Bank statements",
			detail: "Optional for this facility",
			done: true,
			static: true,
		},
		{
		...
		{
			label: "TIN / Ghana Card",
			detail: "Tax identification",
			done: true,
			static: true,
		},
		{
			label: "Tenancy agreement",
			detail: "Premises: rented",
			done: true,
			static: true,
		},
		{
...
export function getCollapsedSteps(state: AppState) {
	return [
		{
			n: 2,
			title: "Upload a current stock list",
			done: state.resolved.stockList,
		},
		{
			n: 3,
			title: "Request the missing MoMo statement window",
			done: state.resolved.coverage,
		},
