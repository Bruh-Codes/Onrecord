		...
		() => ({
			...
			classifyOneOff: (role) =>
				setState((s) => ({
					...s,
					oneOffRole: role,
					resolved: { ...s.resolved, oneOff: true },
				})),
			undoOneOff: () =>
				setState((s) => ({
					...s,
					oneOffRole: null,
					resolved: { ...s.resolved, oneOff: false },
				})),
			toggleDraftMode: () =>
				setState((s) => ({ ...s, draftMode: !s.draftMode })),
			...
			addDeclaredFact: (text) => {
				...
				setState((s) => ({
					...s,
					declaredFacts: [trimmed, ...s.declaredFacts],
				}));
			},
			setReviewStatus: (id, status) =>
				setState((s) => ({
					...s,
					reviewStatuses: { ...s.reviewStatuses, [id]: status },
				})),
			startChat: () =>
				setState((s) =>
					s.chatLog.length === 0
						? {
								...s,
								chatLog: [{ fromAgent: true, text: CHAT_SCRIPT[0].question }],
							}
						: s,
				),
			chooseChatReply: (choiceIndex) =>
				setState((s) =>
					advanceChat(
						s,
						CHAT_SCRIPT[s.chatIndex].choices[choiceIndex].label,
						CHAT_SCRIPT[s.chatIndex].choices[choiceIndex].response,
					),
				),
